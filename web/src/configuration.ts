 ...
 */
export function rehydrateRegexps(rawConfiguration: { [key: string]: any }): monaco.languages.LanguageConfiguration {
    for (const property of REGEXP_PROPERTIES) {
        const value = getProp(rawConfiguration, property)
        if (typeof value === 'string') {
            setProp(rawConfiguration, property, new RegExp(value))
        }
    }
    return rawConfiguration
}

function getProp(obj: { [key: string]: any }, selector: string): any {
    const components = selector.split('.')
    return components.reduce((acc, cur) => (acc != null ? acc[cur] : null), obj)
}

function setProp(obj: { [key: string]: any }, selector: string, value: RegExp): void {
    const components = selector.split('.')
